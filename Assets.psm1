...
if (-not $cmd) {Write-Host -ForegroundColor Green "$command is not a valid command, function, alias or cmdlet."; return}
...
""; Write-Host -f cyan "Command: " -NoNewLine; Write-Host -f yellow $callcommand; Write-Host -f cyan "Source: " -NoNewLine; Write-Host -f yellow $source; Write-Host -f yellow ("-"*100)
foreach ($line in $definition) {if ($line -match "^(.*?)(#.*)#?$") {Write-Host $matches[1] -ForegroundColor White -NoNewline; Write-Host $matches[2] -ForegroundColor Yellow}
elseif ($line -match "(?i)^\s*(sal|set-alias)\b") {Write-Host $line -ForegroundColor Cyan}
...
function font ($newcolour) {[console]::foregroundcolor = "$newcolour"}
...
""; Write-Host -f Green (Resolve-Path $resource).Path; Write-Host -f Yellow ("-"*100); $lineCount = 0; $pauseAfter = 30; $content = Get-Content $resource
...
$lineCount++
if ($lineCount -ge $pauseAfter) {""; Write-Host -f green ("-"*100); font green; $lineCount = 0; $input = Read-Host "Press [Enter] to continue, A to view the whole file, E to Edit or Q to quit"
switch -Regex ($input) {'^(?i)q$' {font gray; ""; return}; '^(?i)a$' {$pauseAfter = 10000}; '^(?i)e$' {font gray; ""; edit $resource; return}}}}
